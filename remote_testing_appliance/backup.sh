...
echo "rsync -ab --compress --stats --no-blocking-io --outbuf=LINE --recursive --files-from='~/scripts/backup_files.txt' --backup-dir=backup_$SUFFIX --delete --exclude=backup --filter='protect backup_*' /home/$user/ remote_backup:backups/ | tee -a /tmp/autobackup_rsync.log" >> ~/scripts/autobackup.sh
